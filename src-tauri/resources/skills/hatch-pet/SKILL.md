...
Inside LevelUpAgent, the application prepares the run before the first provider turn. Do not repeat preparation. For each pending job, call `generate_images` with its exact `hatchJobId` and `hatchRunDir`; the adapter loads `prompt_file` as the authoritative image prompt and loads the job's input images. A short job description is sufficient for the tool's `prompt` argument. Do not read prompt files with `read_file`, `Get-Content`, `cat`, or inline Python. This adapter behavior replaces the manual prompt-file reading in the standalone examples below. Use `finalize_pet_run.py` for extraction, composition, validation, previews, and packaging rather than calling those lower-level scripts separately.
