...
本轮基于 `1.0.56`，尚未发布。已落实模式权限修复、会话摘要分页与正文按需加载、增量保存、全文字面检索、独立草稿、命令面板、渐进历史、工作区拆包、行区间读取和根目录 AGENTS。
范围和 16/20 功能覆盖口径见 [对照清单](REPLACEMENT_AUDIT.md)，实测见 [审阅记录](AGENT_REVIEW_2026-09.md)。

| 优先级 | 下一步 | 验收要求 |
| --- | --- | --- |
| P1 | 重启后未注入队列恢复 | 保留原文，用户明确恢复，不能重放未知工具 |
| P1 | 嵌套项目指令 | 来源与作用域可见，预算有界，不越过选定目录权限 |
| P1 | 主任务 Git worktree 管理 | 新建/删除/迁移、脏树处理、分支与任务关联，真实仓库测试 |
| P1 | 更完整的任务恢复 | 真实 Provider 中断/审批/关闭/重启场景；不只依赖状态单测 |
| P2 | 主 App 与主机继续按领域拆分 | 沿会话、审批、连接设置和工具调度边界提取，避免大范围一次重写 |
| P2 | 超大历史搜索与内存 | FTS 与消息分页；10,000 会话/超长单会话延迟和 RSS 基准 |
| P2 | PTY 终端 | stdin、resize、取消与进程树清理；跨平台生命周期验证 |
| P2 | 持久化自动化 | 调度、时区、休眠/唤醒、权限、去重与通知收件箱 |
| P3 | SSH/云交接 | 独立威胁模型、远程凭据与文件同步契约 |
| 发布 | 平台与升级 | Windows 正式签名更新、macOS notarization、Linux/macOS 实体机回归 |

## 历史里程碑

以下保留版本迭代记录，勾选反映当时状态；不代表本轮重新完成跨平台、付费 Provider 或发行签名验证。
...
- [x] 写入与命令审批
- [x] 规划模式只读隔离
- [x] 工作区路径边界与工具循环终止控制（普通会话和子 Agent 无固定轮次上限）
- [x] 明暗主题、宽窄窗口视觉验证
- [x] 跨平台图标资产
