...
- [x] 文件附件和上下文选择（图片、可可靠解码的文本/代码、PDF 与 Office）
...
- [x] 文本/代码附件按内容和编码识别、疑似二进制拒绝、20 MiB 单文件和 48 MiB 请求总量
- [x] 二进制 FBX 原始文件附件，64 MiB 单文件和请求总量，工作区副本供本地工具读取
...
- [x] 小说、剧本和游戏剧情项目独立于会话持久化，桌面使用 SQLite，Web 预览使用本地回退
- [x] 正文流式 AI 补全、停笔自动触发、选区改写类操作、接受/拒绝/重试与版本快照
- [x] 人物、地点、阵营、物品、世界观、剧情、规则、任务和自定义设定的关系网络与相关度召回
- [x] 场景、对白、选择、条件、结局节点，以及剧情变量、条件/效果解释器、完整性检查和试玩
- [x] JSON/Markdown/Yarn 导入导出、异常项目防御性修复、16 MiB 后端边界和受限文件扩展名
- [x] 1280、768、390 宽度真实浏览器验收；窄屏创作切换、导航抽屉和上下文入口均可达

## 0.14 星图创作编排

- [x] 写作、图像、视频和语音四项标准能力的类型化 DAG 编排与分层并行执行
- [x] 框选节点保存个人蓝图、内部连线提取、全新 ID 实例化和防御性导入
- [x] 画板标注、透明 PNG 蒙版、10%/25%/50% 扩边和 OpenAI-compatible 图片编辑后端
- [x] 内置灵感成片、有声故事卡、智能扩图和蒙版局部重绘四套成熟流程
- [x] 1280、768、390 宽度真实浏览器验收；类型连线、命令面板、撤销/重做和窄屏抽屉均可用
