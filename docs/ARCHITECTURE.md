...
用户通过系统文件选择器、会话输入框 Ctrl+V 或窗口拖放导入附件。Rust 校验普通文件：图片支持 PNG/JPEG/WebP/GIF 魔数、64 MiB
...
MIME 展示，不作为准入条件；PDF/OOXML 扩展名会触发对应结构校验，已知二进制签名与低置信度旧编码
内容仍拒绝。文本限制为每文件 20 MiB、每请求 48 MiB；PDF 与 OOXML 文档支持 PDF/DOCX/XLSX/PPTX，限制
为每文件 20 MiB、每消息 8 份和每请求 48 MiB。二进制 FBX 按文件头识别并保留原始字节，单文件和每请求均为 64 MiB；
发送时在选定工作区的 `.levelup-attachments/<ID>.fbx` 中创建工具可读取的副本，后续工具循环保留已有副本的修改。
此目录自动添加 Git 忽略规则，并排除在工作区变更快照外；文件内容不解码、不作为图片或 Base64 发给文字模型，
上下文只包含文件引用和工作区相对路径。OOXML 拒绝路径逃逸、重复路径、冲突包类型、超过
...
历史附件不再展开，只注入包含名称、类型和大小的保留标记；FBX 另外保留工作区副本路径，便于后续轮次继续处理。
