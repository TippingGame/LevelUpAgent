...
增加不含正文的 Provider 请求日志。Skill 正文与图片二进制不进入数据库。持久化遵循以下原则：
...
用户通过系统文件选择器导入附件。Rust 校验普通文件：图片支持 PNG/JPEG/WebP/GIF 魔数、20 MiB
...
GitHub Secret 进入构建进程。Windows runner 必须导入 Authenticode PFX 并匹配 thumbprint；macOS
runner 必须提供 Developer ID 与 notarization 凭据。缺任一必需值时发布在打包前失败。运行时只
通过 Tauri updater 验证签名、下载并被用户显式点击后重启，不实现任意 URL 下载或跳过签名校验。
...
每次模型响应由 Rust 记录输入/输出 Token 与回合数；达到预算后自动暂停。前端只在 Active 或
Auditing 状态生成隐藏的内部继续消息，并有单次 48 回合上限。
