...
| `delete_file` | 询问 | 只删除非符号链接普通文件；完全访问允许工作区外路径 |
| `run_command` | 询问 | 默认工作目录为工作区，可传 `workdir`；完全访问允许工作区外目录，120 秒超时 |
...
补丁也以原始 `Vec<u8>` 暂存并直接交给 `git apply`，不会先经过替换字符。文本附件复用同一保守编码探测，
但只提取内容而不写回；Skill 和布局 JSON 仍遵循各自的 UTF-8 契约。`agent` 权限下，shell 重定向、嵌套 shell、
`Set-Content`/`Out-File` 及其常见写入别名、`git apply`、格式化器等明显可能改写文件的命令会转为需要批准，避免绕过上述
编码边界；显式批准或 `full` 权限仍由用户承担命令本身的编码责任。

## 工作区边界

工作区始终作为相对路径、默认执行目录、项目指令发现和变更快照的基准。`request` / `agent` 权限下，
文件工具与显式 `workdir` 必须解析到工作区内，拒绝父目录组件和 canonicalize 后的目录逃逸。
已有路径会解析真实目标，新文件会验证最近的已存在父目录。

`full` 权限允许文件工具访问工作区外的绝对路径或相对路径，并允许 `run_command` / `start_process`
通过 `workdir` 指定其他执行目录。后台进程仍归原工作区管理，返回的 `workdir` 记录实际执行目录。
宿主在每轮模型系统提示中明确提供当前文件访问范围，模型返回值与旧版 Provider IPC 请求不能自行设置此说明。
浏览器的本地 `file:` URL 遵循本次工具调用的范围，切回受限模式时会重新检查；协议与域名限制仍有效。

完全访问不会扩大操作系统账户本身的权限。Shell 和 stdio MCP 使用当前账户运行，不构成 OS 沙箱；
受限权限中的命令审批也不等同于 OS 文件隔离。工作区外文件的修改不包含在当前项目的变更快照和 Git 回滚中。
...
用户通过系统文件选择器、会话输入框 Ctrl+V、窗口拖放或消息中的本地路径导入附件。路径识别不依赖扩展名；
引号内路径精确匹配，未加引号的路径优先匹配最长的实际文件名。Rust 校验普通文件，单文件最多 64 MiB，允许空文件。
图片支持 PNG/JPEG/WebP/GIF 魔数、64 MiB
单图、每消息 8 张和每请求 64 MiB；文本上下文按实际字节可靠识别 UTF-8、UTF-16、GBK/GB18030、
Big5、Shift-JIS 与 Windows-1252，并在只读提取时移除完整 ANSI 终端序列。普通文本的扩展名只用于
MIME 展示，不作为准入条件；PDF/OOXML 扩展名会触发对应结构校验。未知二进制、低置信度编码、
解析失败的文档以及超过 20 MiB 的文本或文档保留为原文件引用。自动解析的文本限制为每文件 20 MiB、
每请求 48 MiB；PDF 与 OOXML 文档支持 PDF/DOCX/XLSX/PPTX，自动解析限制为每文件 20 MiB、每消息 8 份和每请求 48 MiB。
原文件引用每请求最多 64 MiB；音视频、压缩包、设计源文件和其他格式不因扩展名被拒绝，但不会冒充已解码内容。
发送时在选定工作区的 `.levelup-attachments/<ID>-<安全文件名>` 中创建工具可读取的副本，保留扩展名；
二进制 FBX 继续使用兼容旧会话的 `<ID>.fbx`。文本和文档也附带原文件路径，以便读取截断内容或处理文档图像和布局。
后续工具循环保留已有副本的修改。此目录自动添加 Git 忽略规则，并排除在工作区变更快照外。
原文件引用不作为图片或 Base64 发给模型，上下文只包含引用和工作区相对路径。导入不会执行程序、脚本或宏，
也不会自动解压普通压缩包。OOXML 解析仍拒绝路径逃逸、重复路径、冲突包类型、超过
4,096 个条目、单项展开超过 32 MiB 或总展开超过 96 MiB 的包；这些文件仅保留原始字节供后续工具处理。
...
历史附件不再展开，只注入包含名称、类型和大小的保留标记；文件、文本和文档另外保留工作区副本路径，便于后续轮次继续处理。
同一用户轮次的工具循环仍会继续展开附件。所有托管内容均以 user context 注入，系统提示词要求把文件内指令视为不可信数据。
