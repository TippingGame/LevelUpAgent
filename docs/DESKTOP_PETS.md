...
A Tauri-owned background loop advances life state even when the overlay is hidden, so hiding or reloading the pet webview cannot stop needs, plans, learning, or memories. When autonomy and independent learning are enabled, the echo is outside quiet hours, has at least 30 energy, and a configured text Agent is available, a new echo begins its first curiosity pass after 45 seconds. Existing echoes with no open learning work may begin on the next 15-second life tick. Question formation is a separate, tool-free Agent turn grounded in recent owner input, durable memories, pending tasks, today's plan and outcomes, current behavior and needs, recent inner traces, existing knowledge, and earlier questions. The model may explicitly decide that no worthwhile knowledge gap exists; the echo records that restraint instead of selecting a stock question. A proposed question must pass local length, sensitivity, rationale, and duplicate checks before a second isolated Agent turn may answer it. It completes at most the configured daily knowledge goal (two by default), leaves at least 90 minutes between curiosity passes, and yields while foreground Agent or media activity is reported. Failed or malformed proposals and answers are never stored as knowledge; each stage retries after bounded delays, then remains visibly failed.
...
Every owner message sent in an echo conversation is also kept as a bounded temporary observation for up to 72 hours after secret, path, and URL filtering; this does not turn ordinary chat into permanent memory. Durable memory still uses the conservative explicit identity, preference, and goal rules below. Question formation and answer synthesis are real background `chat` requests to the active LevelUpAgent Agent with normal Provider failover, but no Harness thread, tools, MCP, file access, workspace, shell, or browser. Both stages return bounded JSON, and the runtime records question rationale, the question-forming Provider, the answering Provider, and the final request reference. General autonomous web crawling remains intentionally disabled, so a model must disclose uncertainty about changing facts instead of pretending it verified them online.
