...
- autonomous curiosity passes with question-forming, deferred, asking, retrying, completed, or failed state, rationale, and both Provider IDs;
- a free desktop position, daily favorite-corner visits, dreams, discoveries, and inner traces;
- separate autonomy, independent-learning, movement, reminders, quiet-hours, patrol-speed, and login-startup settings; public-web tools are an internal capability of independent learning rather than another user-facing mode.
...
A Tauri-owned background loop advances life state even when the overlay is hidden, so hiding or reloading the pet webview cannot stop needs, plans, learning, or memories. When autonomy and independent learning are enabled, the echo is outside quiet hours, has at least 30 energy, and a configured text Agent is available, it begins a curiosity pass only when there is a real signal. Fresh owner context can trigger a context question after a short cooldown; accumulated knowledge and high curiosity can trigger deeper connections or independent exploration; a periodic identity signal is grounded in the echo's own package name, description, and personality. The configured daily value is a cap rather than a quota, and declined question-forming passes also count toward a bounded daily request budget.

Question formation is a separate, tool-free Agent turn grounded in the selected curiosity mode, the echo's stable identity, recent owner input, durable relational memories, pending tasks, today's plan and outcomes, current behavior and needs, recent inner traces, existing knowledge, and earlier questions. The model may explicitly decide that no worthwhile knowledge gap exists; the echo records that restraint instead of selecting a stock question. A proposed question must pass local length, sensitivity, rationale, and duplicate checks before a second bounded Agent run may answer it. Failed or malformed proposals and answers are never stored as knowledge; each stage retries after bounded delays, then remains visibly failed.

Each learning stage has an eight-minute total deadline and runs separately from the 15-second life tick. Only one background learning stage runs at a time, including while switching echoes. Ordinary failures, stale requests, and interrupted requests recovered on startup or backup restore share the same three-attempt limit and 20/40-minute retry delays. Recovery is persisted immediately. Claims and completion states become visible only after the state file is committed, and usage stays attached to the echo that made the request.
...
Every owner message sent in an echo conversation is also kept as a bounded temporary observation for up to 72 hours after secret, path, and URL filtering; this does not turn ordinary chat into permanent memory. Durable memory still uses the conservative explicit identity, preference, and goal rules below. Question formation is a background `chat` request; answer synthesis uses a restricted background Agent with normal Provider failover, no Harness thread, MCP, file access, workspace, shell, or browser, and only the host's `web_search` and `web_fetch` definitions. Both stages return bounded JSON, and the runtime records question rationale, curiosity mode, the question-forming Provider, the answering Provider, and the final request reference.

Different knowledge entries may cite the same public URL without overwriting each other; web knowledge is deduplicated by its title and summary, not by URL alone.

The answer Agent decides whether public evidence is useful; there is no separate web-research switch or host-generated search pass. Each question permits at most one search returning five results and two bounded page fetches, and pages may be fetched only from that search result set. The host rejects queries containing secret/path/URL markers, email-like data, long digit sequences, or eight-character fragments copied from recent owner observations, durable memories, task titles, or task notes. Search and page fetches reuse the host-owned network boundary that rejects private destinations, caps response bytes and text, and labels external content as untrusted. Web-backed knowledge records a human-readable source label and first public source URL; otherwise the entry remains Agent-synthesized and must not claim current verification. Providers that reject tool calling automatically receive one tool-free fallback request.
