...
通过 LevelUpAPI 使用时，上游轮询和完成扣费由网关后台持续负责，应用只查询网关并下载结果。上游 `POST /v1/videos` 的短耗时是提交耗时，并不代表视频已完成；网关记录包含排队、生成及等待查询确认的时间。网关收到成功状态后即结算，客户端下载速度不决定是否扣费。以上客户端修复需重新构建桌面端，单独更新网关不能更新已安装的应用。
