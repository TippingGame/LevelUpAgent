...
| 模式 | 最大连续轮数 |
| --- | ---: |
| Durable Harness 非 Goal 模式 | 无固定上限 |
| Goal | 无固定上限 |
| 隔离子 Agent | 无固定上限 |

普通会话和隔离子 Agent 持续执行工具循环，直到模型不再请求工具或运行因取消、权限处理、错误等条件结束；`delegate_task` 不再提供 `maxTurns` 参数。
...
- 图片编辑、扩图和局部重绘按源图分别提交；每次只发送当前源图及独立参考图列表，使用当前源图自己的蒙版、扩边与标签。Provider 提示词区分“源图 N”和“参考图 N”，源图始终位于第一附件，蒙版只绑定该源图。
- 编辑画板为每张源图保留原始图片及独立笔画、标签和扩边状态；应用后再次打开会恢复可编辑图层，避免重复合成标签或扩边。
