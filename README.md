...
`1.0.53` 是当前发布里程碑。模型连接显示服务返回的全部模型，并在底部提供“打开日志目录”。消息中的本地文件路径与选择、拖入附件共用导入流程，支持任意扩展名、无扩展名和空文件；常见图片、文本及 PDF/Office 自动读取，其他格式保留原文件供本地工具处理。完全访问模式允许文件、命令和浏览器访问工作区外路径，受限模式仍保留范围校验。后台进程结束后保留有界的输出与状态记录，失效进程 ID 不再中断整个任务；上游流读取错误增加重连识别和清晰提示。Windows 安装包仍未配置 Authenticode，macOS 安装包仍未公证。提交 Issue 时请附上复现步骤、应用日志和平台信息。
