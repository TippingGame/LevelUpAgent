...
Download the package for your platform from GitHub **Releases**. The current `1.0.26` Windows x64 installers are built by GitHub Actions and signed with the Tauri updater key; they are not Authenticode-signed, so SmartScreen may show an unknown-publisher warning.
