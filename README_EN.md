...
Download the package for your platform from GitHub **Releases**. The current `1.0.41` Windows x64 installers are built by GitHub Actions and signed with the Tauri updater key; they are not Authenticode-signed, so SmartScreen may show an unknown-publisher warning.
...
`1.0.41` is the current release milestone. Base64 transport data for image attachments is no longer charged as ordinary text in the context budget, preventing large images from displacing later tool results and reducing repeated tool-call loops on compatible models. Model connection protocol labels now include GLM, Kimi, and DeepSeek according to LevelUpAPI ingress compatibility; OpenCode uses sky blue, while the new platform labels use their brand colors. Conversation mode buttons keep Chinese labels on one horizontal line in narrow windows, preventing vertical overflow. Windows installers use Tauri updater signatures but are not Authenticode-signed, so SmartScreen may show an unknown-publisher warning. When filing an issue, include reproduction steps, application logs, and platform details.
